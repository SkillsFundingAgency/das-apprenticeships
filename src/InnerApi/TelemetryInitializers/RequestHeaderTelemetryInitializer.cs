...
namespace SFA.DAS.Apprenticeships.InnerApi.TelemetryInitializers;
...
public class RequestHeaderTelemetryInitializer : ITelemetryInitializer
{
    ...
    public void Initialize(ITelemetry telemetry)
    {
        ...
        foreach (var headerName in requestHeaders)
        {
            ...
            if (headers.Any())
            {
                telemetry.Context.GlobalProperties.Add($"REQ_{headerName}", string.Join(Environment.NewLine, headers));
            }             
        }
        foreach (var headerName in responseHeaders)
        {
            ...
            if (headers.Any())
            {
                telemetry.Context.GlobalProperties.Add($"RESP_{headerName}", string.Join(Environment.NewLine, headers));
            }
        }
    }
}