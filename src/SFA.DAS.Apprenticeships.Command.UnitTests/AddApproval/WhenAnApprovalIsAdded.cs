...
namespace SFA.DAS.Apprenticeships.Command.UnitTests.AddApproval
{
    ...
    public class WhenAnApprovalIsAdded
    {
        ...
        public async Task ThenTheApprovalIsCreated()
        {
            ...
            _apprenticeshipFactory.Setup(x => x.CreateNew(command.Uln, command.TrainingCode, command.DateOfBirth, command.FirstName, command.LastName, command.TrainingPrice, command.EndPointAssessmentPrice, command.AgreedPrice, command.ApprenticeshipHashedId)).Returns(apprenticeship);
