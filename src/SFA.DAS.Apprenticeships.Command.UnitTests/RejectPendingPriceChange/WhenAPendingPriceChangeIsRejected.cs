...
namespace SFA.DAS.Apprenticeships.Command.UnitTests.RejectPendingPriceChange
{
    ...
    public class WhenAPendingPriceChangeIsRejected
    {
        ...
        public async Task ThenPriceHistoryIsCancelled()
        {
            ...
            apprenticeship.AddPriceHistory(_fixture.Create<decimal>(), _fixture.Create<decimal>(), _fixture.Create<decimal>(), _fixture.Create<DateTime>(), _fixture.Create<DateTime>(), PriceChangeRequestStatus.Created, _fixture.Create<string>(), _fixture.Create<string>(), _fixture.Create<string>());

            _apprenticeshipRepository.Setup(x => x.Get(command.ApprenticeshipKey)).ReturnsAsync(apprenticeship);
            
            await _commandHandler.Handle(command);
            
            _apprenticeshipRepository.Verify(x => x.Update(It.Is<ApprenticeshipDomainModel>(y => y.GetEntity().PriceHistories.Count(z => z.PriceChangeRequestStatus == PriceChangeRequestStatus.Rejected) == 1)));
        }
    }
}
