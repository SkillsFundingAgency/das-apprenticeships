...
namespace SFA.DAS.Learning.Command.UnitTests.CancelPendingPriceChange
{
    [TestFixture]
    public class WhenAPendingPriceChangeIsCancelled
    {
        ...
        public async Task ThenPriceHistoryIsCancelled()
        {
            ...
            _apprenticeshipRepository.Verify(x => x.Update(It.Is<LearningDomainModel>(y => y.GetEntity().PriceHistories.Count(z => z.PriceChangeRequestStatus == ChangeRequestStatus.Cancelled) == 1)));
        }
    }
}
